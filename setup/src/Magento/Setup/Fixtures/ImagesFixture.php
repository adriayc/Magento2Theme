...
class ImagesFixture extends Fixture
{
    ...
    /**
     * Create images file and add it to media gallery table in DB
     *
     * @return void
     */
    ...
    /**
     * Create generator that creates images files and puts them to appropriate media folder
     * in memory-safe way
     *
     * @return \Generator
    ...
    private function assignImagesToProducts()
    {
        ...
        foreach ($this->getProductGenerator() as $productEntity) {
            for ($imageNum = 1; $imageNum <= $this->getImagesPerProduct(); $imageNum++) {
                ...
                if ($imageNum === 1) {
                    $attributes = ['images', 'small_image', 'thumbnail', 'swatch_image'];
    ...
    /**
     * Creates generator to iterate infinitely over all images entities
     *
     * @return \Generator
     */

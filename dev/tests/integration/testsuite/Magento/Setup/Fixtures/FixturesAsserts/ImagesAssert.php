...
class ImagesAssert
{
    ...
    private function assertProductMediaGallery(\Magento\Catalog\Model\Product $product)
    {
        ...
        if (count($mediaGalleryImages) !== 1) {
            throw new \AssertionError('Product supposed to contain one images');
        }
     ...
     * e.g. images|small_image|swatch_image|thumbnail
     *
     * @param \Magento\Catalog\Model\Product $product
     * @throws \AssertionError
     */
    ...
    /**
     * Performs assertions over images files in FS
     *
     * @param \Magento\Catalog\Model\Product $product
     * @throws \AssertionError
     */
    private function assertProductImageExistsInFS(\Magento\Catalog\Model\Product $product)
    {
        ...
        if (!$mediaDirectory->isExist($this->mediaConfig->getBaseMediaPath() . $mediaAttributes['images'])) {

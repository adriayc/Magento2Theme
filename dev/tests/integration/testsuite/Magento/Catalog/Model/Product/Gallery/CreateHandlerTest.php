...
class CreateHandlerTest extends \PHPUnit\Framework\TestCase
{
    ...
    private $fileLabel = 'Magento images';
    ...
    public function testExecuteWithImageDuplicate()
    {
        ...
        $product->setData(
            'media_gallery',
            ['images' => ['images' => ['file' => $this->fileName, 'label' => $this->fileLabel]]]
        );
        $product->setData('images', $this->fileName);
        $this->createHandler->execute($product);
        $this->assertStringStartsWith('/m/a/magento_image', $product->getData('media_gallery/images/images/new_file'));
        $this->assertEquals($this->fileLabel, $product->getData('image_label'));

        $product->setIsDuplicate(true);
        $product->setData(
            'media_gallery',
            ['images' => ['images' => ['value_id' => '100', 'file' => $this->fileName, 'label' => $this->fileLabel]]]
        );
        $this->createHandler->execute($product);
    ...
    /**
     * Check sanity of posted images file name
     *
    ...
    public function testExecuteWithIllegalFilename($imageFileName)
    {
        ...
        $product->setData(
            'media_gallery',
            ['images' => ['images' => ['file' => $imageFileName, 'label' => 'New images']]]
        );
        $product->setData('images', $imageFileName);
    ...
    public function testExecuteWithImageRoles($image, $smallImage, $swatchImage, $thumbnail)
    {
        ...
        $product->setData(
            'media_gallery',
            ['images' => ['images' => ['file' => $this->fileName, 'label' => '']]]
        );
        $product->setData('images', $image);
        ...
        $this->assertEquals(
            $image,
            $resource->getAttributeRawValue($id, $resource->getAttribute('images'), $storeId)
        );
        $this->assertEquals(
            $smallImage,

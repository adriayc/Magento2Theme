...
class ProcessorTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testUpdateImage()
    {
        ...
        $product->setData('media_gallery', ['images' => ['images' => ['file' => 'magento_image.jpg']]]);
        $this->_model->updateImage($product, 'magento_image.jpg', ['label' => 'test label']);
        $this->assertEquals('test label', $product->getData('media_gallery/images/images/label'));
    }
    ...
    public function testRemoveImage()
    {
        ...
        $this->assertEquals('1', $product->getData('media_gallery/images/images/removed'));
    }
    ...
    public function testClearMediaAttribute()
    {
        ...
        $product->setData(['images' => 'test1', 'small_image' => 'test2', 'thumbnail' => 'test3']);

        $this->assertNotEquals('no_selection', $product->getData('images'));
        $this->_model->clearMediaAttribute($product, 'images');
        $this->assertEquals('no_selection', $product->getData('images'));
    ...
    public function testSetMediaAttribute()
    {
        ...
        $this->_model->setMediaAttribute($product, 'images', 'test1');
        $this->assertEquals('test1', $product->getData('images'));

        $this->_model->setMediaAttribute($product, ['non-exist-images-attribute', 'small_image'], 'test');
        $this->assertNull($product->getData('non-exist-images-attribute'));
